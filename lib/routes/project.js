module.exports = function(modulus) {
  ...
  help.add('deploy', function() {
    ...
    this.line('    -n, --node-version    Specifies the node version to use when demeteorizing a meteor project, if applicatable.');
    this.line('    -t, --project-name    Specifies type of application you are deploying. Overrides the auto-detection that occurs normally. Acceptable types are listed below.');
    this.line('      nodejs   A typical Node.js application.');
    this.line('      meteor   A meteor application, which requires demeteorization before deploy.');
  });
  ...
  modulus.program
    ...
    .option('-n, --node-version [value]', 'Specifies the node version to use when demeteorizing the project, if applicatable.')
  ...
  modulus.program
    ...
    .action(function(dir){
      modulus.runCommand(modulus.commands.project.deploy, [modulus.program.projectName, dir, modulus.program.projectType, modulus.program.includeModules, modulus.program.nodeVersion], true);
    });
