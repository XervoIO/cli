...
Project.prototype.deploy = function (projectId, dir, includeModules, options, callback) {
  ...
  async.waterfall([
    ...
    function (cb) {
      ...
      var srcVersionPath = path.join(dir, '.meteor', 'release')
      fs.readFile(srcVersionPath, { encoding: 'utf8' }, function (err, version) {
        ...
        xervo.io.print('Deploying Meteor version ' + version)
        cb()
      })
    },
    function (cb) {
      xervo.io.print('Compressing project...')
...
Project.prototype._deploy = function (projectId, file, options, callback) {
  xervo.io.print('Uploading project...')
    ...
    var getDeployLogs = function (cb) {
      ...
      librarian.project.getDeployLogs(projectId, userConfig.data.apiKey, function (err, result) {
        for (var key in result) {
          var log = result[key]
          xervo.io.write(log.substring(logLength))
...
Project.prototype.streamLogs = function (projectId, servos, callback) {
  ...
  xervo.io.success('Log streaming started (ctrl-c to exit)...')
...
// -----------------------------------------------------------------------------
Project.prototype.scale = function (projectId, instances, wait, callback) {
...
Project.prototype.download = function (projectId, projectName, outLocation, callback) {
  librarian.project.download(projectId, userConfig.data.apiKey, function (err, req, res) {
    var message, date, outFile, dir, stream
    var result = {}
...
Project.prototype.packageProject = function (dir, includeModules, cb) {
  var fname = os.tmpdir() + '/' + 'xervo-auto-zip-' + uuid.v4() + '.zip'
  ...
  var ignore = Ignore({
    path: dir,
    ignoreFiles: ['.xervo-base-ignore', '.xervoignore']
  })

  // Copy the base ignore file to the output so it can be applied.
  fs.writeFileSync(path.join(dir, '.xervo-base-ignore'),
    fs.readFileSync(path.join(__dirname, '.xervo-base-ignore')))

  // Add node_modules to ignore file if user did not want to include it.
  if (!includeModules) {
    fs.appendFileSync(path.join(dir, '.xervo-base-ignore'), '\nnode_modules')
  }
  ...
  ignore.on('error', function (err) {
    error = true
    fs.unlinkSync(path.join(dir, '.xervo-base-ignore'))
    cb(err, fname)
  })
  ...
  out.on('close', function () {
    ...
    xervo.io.print(makeBytesReadable(stats.size.toString()).data + ' written')
