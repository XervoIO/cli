...
//-----------------------------------------------------------------------------
Project.prototype.deploy = function(projectId, dir, includeModules, callback) {
  ...
  async.waterfall([
      ...
      function(type, cb) {
        switch(type) {
          ...
          case Project.types.METEOR:
            ...
            projectController.demeteorize(dir, cb);
            break;
        }
      },
      ...
      function(path, cb) {
        ...
        projectController.packageProject(path, includeModules, function(err, fname) {
      ...
      function(fpath, cb) {
        projectController._deploy(projectId, fpath, function(err, domain) {
          fs.unlinkSync(fpath);
          cb(null, domain);
        });
      }
...
//-----------------------------------------------------------------------------
Project.prototype._deploy = function(projectId, file, callback) {
 ...
 */
//-----------------------------------------------------------------------------
Project.prototype.streamLogs = function(projectId, callback){
  modulus.io.success('Log streaming started (ctrl-c to exit)...');

  var endpoint = util.format('%s://%s:%s/project/%s/logs/stream?authToken=%s',
 ...
 */
//-----------------------------------------------------------------------------
Project.prototype.demeteorize = function(dir, callback) {
  ...
  demeteorizer.convert(dir, out, 'v0.8.24', function(err) {
    if(err) {
      callback(err);
    }
    else {
