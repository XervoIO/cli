...
module.exports = function (packagePath, done) {
  ...
  var blackList = [{word: 'forever', level: 'FATAL', message: 'Your application is currently configured ' + 
    'to run using forever, which is not supported on Modulus. Please change your start script to simply ' + 
    'start the application and we\'ll handle the rest.'},
    {word: 'pm2', level: 'FATAL', message: 'Your application is currently '
    + 'configured to run using pm2, which is not supported on Modulus. Please ' 
    + 'change your start script to simply start the application and we\'ll handle the rest.'}];
  var meta;
  try {
