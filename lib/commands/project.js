...
//
// Find the meta (package.json) content for the project being deployed.
//
var findMeta = function(cb) {
  ...
  walkFiles(process.cwd())
    ...
    .on('file', function(ops) {
      if (ops.name === 'package.json') file = ops.source;
    })
    .start(function(err) {
      if (err || !file) return cb({ code: 'NO_MATCHING_NAME' }, null);
      cb(null, file);
    });
};
...
project.chooseProjectPrompt = function(projects, projectName, cb) {
  ...
  if(typeof projectName === 'string') {
    projectIndex = projectInArray(projectName, projects);
    ...
    findMeta(function(err, path) {
      ...
      if (typeof meta['mod-project-name'] !== 'undefined') {
        ...
        if (projectIndex >= 0) {
          ...
          modulus.io.print('Deploying ' + meta['mod-project-name'] + '...');
