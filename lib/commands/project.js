      ...
      project.chooseProjectPrompt(projects, function(err, result) {
        ...
        if(download){
          ...
          projectController.downloadLogs(result.id, result.name, output, function(err, res) {
            if(!err){
              modulus.io.print(res.msg);
              modulus.io.success('Logs successfully downloaded.');
            } // Do I need to include an error message if there's a problem?
            cb(err);
          });
        } else{
    ...
    function(err, projects) {
      ...
      project.chooseProjectPrompt(projects, function(err, result) {
        ...
        projectController.clearLogs(result.id, function(err, res){
          console.log('clear log completed - ' + err);
          console.log('res: ' + res);
          if(!err){
            //modulus.io.print(res.msg); // might not need this message
