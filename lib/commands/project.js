...
//
// Returns true if Modulus supports servo size, false otherwise
//
        ...
        function resize(fn) {
          projectController.resize(projectInfo.id, servoSize, function (err) {
            if (err) {
              ...
              modulus.io.warning(projectInfo.name + ' must be stopped and started for the changes to take effect.');
              modulus.io.success(projectInfo.name + ' resized at ' + projectInfo.domain);
            }
          });
        }
      ], cb);
...
project.create = function (projectName, servoSize, runtime, projectOwner, cb) {
  var user, userId, projects, organizations, filteredImages, images, imageTagIds = {},
    found, selected;
  ...
  async.series([
    ...
    function promptForProjectOwner(fn) {
      if ('string' === typeof projectOwner && projectOwner.length > 0) {
        found = false;
        selected = null;
