  ...
  projectController.find({
    ...
    function(err, projects) {
      ...
      project.chooseProjectPrompt(projects, projectName, function(err, result) {
        ...
        project._packageProject(dir, includeModules, function(err, zipPath) {
          ...
          projectController.deploy(result.id, zipPath, function(err, domain) {
            fs.unlinkSync(zipPath);
            if(!err) {
...
project.chooseProjectPrompt = function(projects, projectName, cb) {
  var projectIndex = -1, metaPath = null;
