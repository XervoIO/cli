...
//
// Returns true if Xervo supports servo size, false otherwise
//
function validateServoSize (servoSize) {
  if (typeof servoSize !== 'string') {
    return false
  }

  if (['192', '396', '512', '1024', '2048'].indexOf(servoSize) !== -1) {
    return true
  } else {
    return false
  }
...
project.resize = function (projectName, servoSize, cb) {
  var projectInfo
      ...
      async.series([
        function promptForProject (fn) {
          project.chooseProjectPrompt(projects, projectName, function (err, result) {
            ...
            if (!result) {
              return fn('You must select a project.')
            }
        ...
        function promptForServoSize (fn) {
          function handleServoSize (err, result) {
            if (err) return error.handlePromptError(err, cb)

            servoSize = result.servoSize

            if (validateServoSize(result.servoSize)) {
              fn()
            } else {
              fn('Servo size was not one of these choices: 192, 396, 512, 1024, or 2048')
            }
          }

          if (servoSize && servoSize.length > 0) {
            return handleServoSize(null, {servoSize: servoSize})
          }

          xervo.io.prompt.get([{
            name: 'servoSize',
            description: 'Enter a servo size in MB [192, 396, 512, 1024, or 2048]:',
            message: 'Servo size should be a number (192, 396, 512, 1024, 2048)',
            required: true,
            pattern: /^\d+$/
          }], handleServoSize)
        },
        function resize (fn) {
          projectController.resize(projectInfo.id, servoSize, function (err) {
            if (err) {
              fn(err)
            } else {
              xervo.io.warning(projectInfo.name + ' must be stopped and started for the changes to take effect.')
              xervo.io.success(projectInfo.name + ' resized at ' + projectInfo.domain)
            }
          })
        }
      ], cb)
...
project.list = function (cb) {
  projectController.find({
    ...
    function (err, projects) {
      ...
      if (projects.length === 0) {
        xervo.io.print('You currently have no projects. You can create one with "project create".')
        return cb()
      }
      ...
      var owners = {}
      xervo.io.print('Current projects:'.input)
...
project.delete = function (projectName, cb) {
  async.waterfall([
    ...
    function promptForProject (projects, fn) {
      ...
      if (projectName) {
        projectFound = projects.some(function (p) {
          return p.name.toLowerCase() === projectName.toLowerCase()
        })

        if (!projectFound) {
          xervo.io.error('Project not found')
    ...
    function deleteProject (project, fn) {
      projectController.delete(project.id, function (err, result) {
        ...
        xervo.io.success('Project ' + project.name + ' deleted.')
        fn(null, result)
      })
    }
  ], cb)
...
project.create = function (projectName, servoSize, runtime, projectOwner, cb) {
  var user, userId, projects, organizations, filteredImages, images, found,
    selected
  var imageTagIds = {}
  ...
  async.series([
    ...
    function findOrganizationMembers (fn) {
      userId = userConfig.data.userId
      userController.getOrganizations(userId, function (err, results) {
        if (err) {
          err = error.handleApiError(err, 'GET_ORGANIZATION_MEMBERS', fn)
          if (err.length > 0) {
            return fn(err)
          }
        }

        // Only include organizations that the user has admin access to.
        organizations = results.filter(function (org) {
          return org.roles.indexOf('admin') >= 0
        })
    ...
    function promptForProject (fn) {
      if (projectName && projectName !== '') {
        xervo.io.print('Creating project ' + projectName.data)
        return fn()
      }
    ...
    function promptForImage (fn) {
        ...
        images.forEach(function (image) {
          if (image.label.toLowerCase() === label.toLowerCase()) {
            image.tags.forEach(function (tag) {
              if (tag.name === 'latest' && type === image.type) {
                ret = tag.id
              }
            })
          }
      ...
      xervo.io.print('Please choose which runtime to use:'.input)
      filteredImages = images.filter(function (image) {
        return image.type === 'run' && image.official
      })

      filteredImages.forEach(function (image, index) {
        var out = '  ' + (index + 1) + ') ' + image.label

        if (image.stability !== 'stable') {
    ...
    function promptForProjectOwner (fn) {
      if (typeof projectOwner === 'string' && projectOwner.length > 0) {
        found = false
        selected = null

        if (projectOwner.toLowerCase() === userConfig.data.username) {
          projectOwner = userConfig.data.userId
          return fn()
        }

        found = organizations.some(function (org) {
          selected = org.id
          return org.username.toLowerCase() === projectOwner.toLowerCase()
        })
        ...
        return fn([
          'No organization members found that matches specified username.',
          'Make sure the username is correct and that you have access to create projects for the organization.'
        ].join('\n'))
      }
      ...
      xervo.io.print('Please choose the owner for this project:'.input)
      xervo.io.print('  1) ' + userConfig.data.username)
      organizations.forEach(function (org, index) {
        xervo.io.print('  ' + (index + 2) + ') ' + org.username)
      })
      ...
      xervo.io.prompt.get([{
        ...
        required: true,
        minimum: 1,
        maximum: organizations.length + 1
      }],
      function (err, result) {
        if (err) return error.handlePromptError(err, cb)

        if (result.projectOwner === 1) {
          projectOwner = userConfig.data.userId
          xervo.io.print('Assigning project owner: ' + userConfig.data.username.data)
        } else {
          projectOwner = organizations[result.projectOwner - 2].id
          xervo.io.print('Assigning project owner: ' + organizations[result.projectOwner - 2].username.data)
        }
  ...
  function createProject (err) {
    ...
    projectController.create(
      ...
      function (err, createdProject) {
        ...
        if (!createdProject) return cb('Could not create project. Error from server.')

        xervo.io.success('New project ' + projectName.data + ' created.')
        return cb(null, createdProject)
      })
  })
}
 ...
 * @param {string} nodeVersion Version of node to use when running application
 * @param {string} npmVersion Version of npm to use when building application
 * @param {boolean} debug Value indicating whether to pass debug flag to demeteorizer
 * @param {function} cb The callback.
 */
// -----------------------------------------------------------------------------
project.deploy = function (projectName, dir, includeModules, registry, withTests, nodeVersion, npmVersion, debug, cb) {
  var options
    ...
    if (probs && probs.length) {
      ...
      probs.forEach(function (problem) {
        ...
        if (problem.level === 'FATAL') {
          xervo.io.error(problem.message)
        } else {
          xervo.io.warning(problem.message)
        }
      })
  ...
  async.waterfall([
    ...
    function promptForProject (projects, fn) {
      if (projectName) {
        ...
        if (!exists) return fn(null, projects, null)
      }
      project.chooseProjectPrompt(projects, projectName, function (err, selectedProject) {
    ...
    function verifyProjectExists (projects, selectedProject, fn) {
      ...
      xervo.io.prompt.get([{
        ...
        if (answer) {
          return project.create(projectName, null, null, null, function (err, project) {
            if (err) return cb(err)
            fn(null, project)
          })
        }
    ...
    function deployProject (selectedProject, fn) {
      if (!selectedProject) return fn('You must deploy to a project.')
      ...
      projectController.deploy(selectedProject.id, dir, includeModules, options, function (err, domain) {
        if (!err) {
          xervo.io.success(selectedProject.name + ' running at ' + domain)
          fn()
        } else {
          projectController.status(selectedProject.id, function (e, status) {
            ...
            if (status === 'UPLOADING' && uploadTries < uploadTriesMax) {
              uploadTries++
              xervo.io.error(util.format('Upload Failed. Retrying. (%s/%s)', uploadTries, uploadTriesMax))
              project.deploy(selectedProject.name, dir, includeModules, registry, fn)
            } else {
              fn(err)
            }
          })
  ...
  async.waterfall([
    ...
    function handleDeployLogs (project, next) {
      ...
      projectController.getDeployLogs(project.id, function (err, logs) {
        ...
        xervo.io.print(logs.build)
        xervo.io.success('Deploy logs successfully retrieved.')
        next()
      })
    }
  ], cb)
...
project.getLogs = function (projectName, download, output, dir, cb) {
  ...
  projectController.find({
    ...
    function (err, projects) {
      ...
      if (projects.length === 0) {
        xervo.io.error('You currently have no projects. You can create one with "project create".')
        return cb()
      }
      ...
      project.chooseProjectPrompt(projects, projectName, function (err, result) {
        ...
        if (download) {
          projectController.downloadLogs(result.id, result.name, output, function (err, res) {
            if (!err) {
              xervo.io.success(res.msg)
            } else {
              xervo.io.error('Problem downloading logs.')
            }
            cb(err)
          })
        } else {
          projectController.getLogs(selectedProject.id, function (err, logs) {
            ...
            if (logs.length === 1) {
              xervo.io.print(logs[0].log)
              xervo.io.success('Logs successfully retrieved.')
              cb()
              ...
              project.chooseServoPrompt(selectedProject, false, function (err, servo) {
                ...
                for (var i = 0; i < logs.length; i++) {
                  if (logs[i].servo.id === servo.id) {
                    xervo.io.print(logs[i].log)
                    xervo.io.success('Logs successfully retrieved for servo ' + servo.id)
                    return cb()
                  }
                }

                xervo.io.print('No log data.')
              })
            }
          })
        }
    ...
    function (err, projects) {
      ...
      project.chooseProjectPrompt(projects, projectName, function (err, result) {
        ...
        projectController.clearLogs(result.id, function (err) {
          if (!err) {
            xervo.io.success('Logs successfully cleared.')
          } else {
            xervo.io.error('Failed to clear logs.')
          }
          cb(err)
        })
      })
      ...
      project.chooseProjectPrompt(projects, projectName, function (err, result) {
        ...
        var go = function () {
          projectController.streamLogs(selectedProject.id, selectedServos, function () {
            xervo.io.error('Disconnected from stream.')
          })
        }
 ...
 * $ xervo project scale #
 * $ xervo project scale aws.us-east-1a=# joyent.us-east-1=#, etc.
...
project.scale = function (instances, projectName, cb) {
  ...
  if (!instances) {
    xervo.io.error('Please specify the number of servos.')
    xervo.Help.printUsage('scale')
    return cb()
  }
    ...
    function (err, projects) {
      ...
      project.chooseProjectPrompt(projects, projectName, function (err, result) {
        ...
        projectController.scale(
          ...
          function (err) {
            if (!err) {
              xervo.io.success('Project successfully scaled.')
            }
            cb(err)
          })
      })
...
project.runApp = function (projectName, nodeEnv, cb) {
  async.waterfall([
    function getUserProjects (fn) {
      projectController.find({ userId: userConfig.data.userId }, function (err, projects) {
        if (err) return error.handleApiError(err, 'GET_PROJECTS', fn)
        if (projects.length === 0) return error.handleApiError({ code: 'PROJECTS_NOT_FOUND' }, 'GET_PROJECTS', fn)
    ...
    function promptForProject (projects, fn) {
      if (projectName) {
        ...
        if (!exists) return error.handleApiError({ code: 'NO_MATCHING_NAME' }, 'CHOOSE_PROJECT_PROMPT', cb)
      }

      project.chooseProjectPrompt(projects, projectName, function (err, selectedProject) {
        if (err) return error.handlePromptError(err, fn)

        fn(null, selectedProject)
      })
    },
    function getProjectEnvVars (project, fn) {
      if (!project) return fn('You must select a project.')
    ...
    function buildStartCommand (envVars, fn) {
      project.getMainFile(function (err, file) {
        ...
        project._createStartCommand(envVars, nodeEnv, file, function (err, command) {
          if (err) return fn(err)
          fn(null, command)
        })
      })
    ...
    function startApp (command, fn) {
      xervo.io.print('Starting application')
      var program = proc.exec(command)

      if (!program.killed) xervo.io.success('Application started successfully.')
...
project._createStartCommand = function (envVars, nodeEnv, mainFile, cb) {
  ...
  for (var v = 0; v < envVars.length; v++) {
    if (envVars[v].name === 'NODE_ENV') {
      command = command.concat(util.format(' %s=%s', envVars[v].name, 'development'))
    } else {
      command = command.concat(util.format(' %s=%s', envVars[v].name, envVars[v].value))
    }
  }
...
project.getMainFile = function (cb) {
  ...
  var mainFile
  var metaPath = findFileSync(project.dir || process.cwd(), 'package.json', ['.git', 'node_modules'])
  if (metaPath) {
    var meta = null
    ...
    if (meta && meta.main && meta.main !== '') {
      mainFile = meta.main
      xervo.io.print(util.format('Main file found in package.json: %s', mainFile))
      cb(null, mainFile)
    } else {
      cb('Main file not specified in package.json.')
    }
  }
}
    ...
    function (err, projects) {
      ...
      project.chooseProjectPrompt(projects, projectName, function (err, result) {
        ...
        projectController.download(result.id, result.name, output, function (err, res) {
          if (err) {
            xervo.io.error('Problem downloading source.')
            return cb()
          }
...
/**
 * Open the project's URL with the user's default browser.
 * @param {String} projectName Name of the project as specified by the `-p`
 * option.
 * @param {Function} cb Callback function for returning an error.
 */
// -----------------------------------------------------------------------------
project.open = function (projectName, cb) {
  projectController.find({
    userId: userConfig.data.userId
  },
    function (err, projects) {
      if (err) {
        err = error.handleApiError(err, 'OPEN_PROJECT', cb)
      ...
      project.chooseProjectPrompt(projects, projectName, function (err, result) {
        ...
        if (result.status !== 'RUNNING') {
          return cb('Your project is not running. Please start your project to open the URL.')
        }

        xervo.io.print('Opening ' + result.domain)
        require('opener')('http://' + result.domain)
      })
    })
}
...
project.chooseServoPrompt = function (project, multi, cb) {
  ...
  xervo.io.print('Please choose which servo to use:'.input)

  if (multi) {
    xervo.io.print('  0' + ') '.input + 'All')
  }
...
project.chooseProjectPrompt = function (projects, projectName, cb) {
  ...
  if (metaPath && autoCmds.indexOf(project.cmd) >= 0) {
    ...
    if (meta && meta['mod-project-name'] && !projectName) {
      ...
      exists = projects.some(function (p) {
        return p.name.toUpperCase() === meta['mod-project-name'].toUpperCase()
      })

      if (exists) {
        projectName = meta['mod-project-name']
        xervo.io.print('Found mod-project-name \'' + projectName + '\' in package.json')
      }
    }
  }
  ...
  if (typeof projectName === 'string') {
    ...
    if (exists) {
      var proj = projects.filter(function (p) {
        if (p.name.toUpperCase() === projectName.toUpperCase()) return p
      })

      xervo.io.print(util.format(foundPrint, proj[0].name.data))
      return cb(null, proj[0])
    }
  ...
  if (projects.length === 1) {
    ...
    var table = []
    xervo.io.print('Please choose which project to use:'.input)
    ...
    xervo.io.prompt.get([{
      ...
      xervo.io.print(util.format(foundPrint, projects[result.project - 1].name.data))
