  ...
  projectController.find({
    ...
    function(err, projects) {
      ...
      project.chooseProjectPrompt(projects, projectName, function(err, result) {
        ...
        projectController.streamLogs(result.id, function(err, req, res){
          if(res.statusCode == 404){
            modulus.io.error(res.statusCode + ' - problem streaming logs.');
            cb(err);
          }
          else if(!err){
            ...
            modulus.io.error('Failed to stream logs: ' + err);
            cb(err);
          }
