...
addOn.provision = function(projectName, addon, callback) {
  ...
  async.waterfall([
    ...
    function(pro, cb) {
      project = pro;
      addOnController.provision(project.id, addon[0], addon[1], cb);
    },
    ...
    function(addons, cb) {
      for(var a = 0; a < addons.length; a++) {
        if(addons[a].modulus_id === provisioned.modulus_id) {
          provisioned = addons[a];
          break;
        }
      }
      ...
      addOn.printList(project.name, [provisioned]);
      
      console.log();
      modulus.io.success('Add-On ' + provisioned.addon_name + ' provisioned.');
    }
  ], 
  function(err) {
    callback(err);
