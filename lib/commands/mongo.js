        ...
        mongo.chooseRegionPrompt(function (err, regionData) {
          ...
          mongoController.clone(db, hosts, function (err, database) {
            ...
            mongo._createUserOnClonedDB(database, false, function () {
              ...
              mongo._setEnvVarsAndRestartProject(database, false, cb)
            })
          })
        })
      })
...
// ------------------------------------------------------------------------------
mongo._setEnvVarsAndRestartProject = function (database, repeatRun, cb) {
  var prompt = []
  var projectInfo
    ...
    if (results.setEnvVars === 'yes') {
      ...
      async.waterfall([
          ...
          if(results.length === 0) {
            ...
            xervo.io.prompt.get(prompt, function (err, results) {
              ...
              if (results.createEnvVar === 'yes') {
                ...
                return mongo._setEnvVarsAndRestartProject(database, true, cb)
              }
            })
          } else {
        ...
        function restartProject(fn, err) {
          mongo._restartProject(projectInfo.name, fn)
...
// ------------------------------------------------------------------------------
mongo._restartProject = function (project, cb) {
  var prompt = []
  prompt.push({
    name: 'restartProject',
