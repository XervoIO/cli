      ...
      mongo.chooseDatabasePrompt(databases, databaseName, function (err, database) {
        ...
        mongo.chooseRegionPrompt(function (err, regionData) {
          ...
          mongoController.clone(db, hosts, function (err, database) {
            ...
            mongo.createUserOnClonedDB(database, false, function () {
              // Update project env vars to point to db and optionally restart
              mongo.setEnvVarsAndRestartProject(database, false, cb)
            })
          })
        })
      })
...
// ------------------------------------------------------------------------------
mongo.setEnvVarsAndRestartProject = function (database, repeatRun, cb) {
  var prompt = []
  var projectInfo
  ...
  xervo.io.prompt.get(prompt, function (err, results) {
    ...
    if (results.setEnvVars === 'yes') {
      ...
      async.waterfall([
        ...
        function chooseProject (results, fn, err) {
          if (err) {
            return error.handleApiError(err, 'GET_PROJECTS', fn)
          }
        ...
        function getEnvVars (results, fn, err) {
          if (err) {
            return error.handleApiError(err, 'GET_ENV_VARS', fn)
          }
        ...
        function chooseEnvVar(results, fn, err) {
          if (err) {
            return error.handleApiError(err, 'CHOOSE_ENV_VARS', fn)
          }
          ...
          if(results.length === 0) {
            ...
            xervo.io.prompt.get(prompt, function (err, results) {
              ...
              if (results.createEnvVar === 'yes') {
                return fn(null, "MONGO_URL")
              } else {
                // bail out and move to the next project
                return mongo.setEnvVarsAndRestartProject(database, true, cb)
              }
            })
          } else {
            //list vars on project
            env.chooseEnvVarPrompt(results, fn, err)
          }
        },
        function setEnvVar(results, fn, err) {
          ...
          xervo.io.prompt.get(prompt, function (err, results) {
            ...
            return env.set(projectInfo.name, dbEnvVar, envValue, fn)
          })
        },
        function restartProject(fn, err) {
          mongo.restartProject(projectInfo.name, fn)
        }], function (err, results) {
            if (err) {
              return error.handleApiError(err, 'MIGRATE_MONGO', cb)
            }
...
// ------------------------------------------------------------------------------
mongo.restartProject = function (project, cb) {
  var prompt = []
  prompt.push({
    name: 'restartProject',
    description: 'Do you want to restart project ' + project + '?',
    message: 'Acceptable response is "yes" or "no".',
    pattern: /^[yntf]{1}/i,
    default: 'no'
  })
  ...
  xervo.io.prompt.get(prompt, function (err, results) {
    ...
    if (results.setEnvVars === 'yes') {
      return project.restart(project, cb)
    } else {
      return cb()
    }
  })
...
// ------------------------------------------------------------------------------
mongo.createUserOnClonedDB = function (database, repeatRun, cb) {
  var prompt = []
  prompt.push({
    name: 'createDBUser',
      ...
      mongo._userPrompt(null, null, null, function (err, results) {
        ...
        mongoController.createUser(
          ...
          function (err, dbUser) {
            ...
            xervo.io.success('New MongoDB database user ' + results.username.data + ' created.')
            return mongo.createUserOnClonedDB(database, true, cb)
          })
      })
    } else {
      return cb()
