  ...
  if (github) {
    if(prompt.length > 0) {
      modulus.io.prompt.get(prompt, function (err, result) {
        ...
        user.authenticateGithub(login || result.login, pass || result.password, loginProvided, passProvided, cb);
      });
    } else {
      user.authenticateGithub(login, password, loginProvided, passProvided, cb);
    }
  } else {
    if(prompt.length > 0) {
      modulus.io.prompt.get(prompt, function (err, result) {
        ...
        user.authenticate(login || result.login, pass || result.password, loginProvided, passProvided, cb);
      });
    } else {
      user.authenticate(login, password, loginProvided, passProvided, cb);
    }
  }
};
