...
var envVarInArray = function (envVars, name) {
  for (var idx = 0; idx < envVars.length; idx++) {
    if (envVars[idx].name.toUpperCase() === name.toUpperCase()) return idx
  }
...
env.chooseEnvVarPrompt = function (envVars, envVar, cb) {
  ...
  if (envVars.length === 1) {
    ...
    xervo.io.prompt.get([{
      name: 'confirm',
      description: 'Are you sure you want to use environment veriable ' + selectedEnvVar.name.data + '?',
