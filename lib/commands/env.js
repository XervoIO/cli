      ...
      function(data, fn) {
        ...
        for (var i = 0; i < project.envVars.length; i++) {
          if (keys.indexOf(project.envVars[i].name) >= 0) {
            try {
              project.envVars[i].value = JSON.stringify(data[project.envVars[i].name]);
              keys.splice(keys.indexOf(project.envVars[i].name), 1);
            } catch (err) {
        ...
        for (var ii = 0; ii < keys.length; ii++) {
          try {
            project.envVars.push({ name: keys[ii], value: JSON.stringify(data[keys[ii]]) });
          } catch (err) {
            fn({ code: 'INVALID_VALUE' });
          }
        }
