...
env.export = function (projectName, file, done) {
  ...
  env._getAndChooseProject(projectName, function (err, project) {
    ...
    fs.writeFile(filePath, JSON.stringify(envVars, null, 2), function (fsErr) {
      ...
      xervo.io.success('Successfully exported environment variables.')
      done()
    })
  })
}
...
env.list = function (projectName, cb) {
  ...
  async.waterfall([
    ...
    function (pro, cb) {
      project = pro
      xervo.io.print('Project ' + project.name.data + ' Environment Variables')
...
env.set = function (projectName, name, value, cb) {
  xervo.commands.project.cmd = 'env set'
  env._getAndChooseProject(projectName, function (err, project) {
    if (err) { return cb(err) }

    var newEnv = {name: name, value: value}
    var found = false
    xervo.io.print('Setting ' + newEnv.name.yellow + ' for project ' + project.name.data)
    for (var i = 0; i < project.envVars.length; i++) {
    ...
    projectController.saveVars(project.id, project.envVars, function (err) {
      ...
      xervo.io.success('Successfully set environment variable.')
      return cb()
    })
  })
}
...
env.load = function (projectName, file, cb) {
  env._getAndChooseProject(projectName, function (err, project) {
    ...
    function (err) {
      ...
      xervo.io.success('Successfully set environment variables loaded from file.')
    })
  })
}
...
env.delete = function (projectName, name, cb) {
  env._getAndChooseProject(projectName, function (err, project) {
    ...
    var found = false
    xervo.io.print('Deleting ' + name.yellow + ' for project ' + project.name.data)
    for (var i = 0; i < project.envVars.length; i++) {
      if (project.envVars[i].name === name) {
    ...
    projectController.saveVars(project.id, project.envVars, function (err) {
      ...
      xervo.io.success('Successfully deleted variable ' + name.yellow + ' from project ' + project.name.data)
      return cb()
    })
  })
}
...
env._getAndChooseProject = function (projectName, cb) {
  ...
  function (err, projects) {
    ...
    xervo.commands.project.chooseProjectPrompt(projects, projectName, function (err, result) {
      if (err) {
        return cb(err)
      }
...
env._printEnv = function (env) {
  xervo.io.print(env.name.yellow + ' = ' + env.value.grey)
}

// Find the index for the specified env var in the specified env var array.
//    Returns -1 if not found.
//
var envVarInArray = function (envVars, name) {
  var index = _.find(envVars, function (envVar) {
    envVar.name.toUpperCase() === name.toUpperCase()
  })
...
// ------------------------------------------------------------------------------
/**
 * Displays a prompt for choosing an envVar from a list.
 * @param {Array} envVars Array of a project envVars.
 * @param {String} [envVar] Default selection.
 * @param {Function} cb
 */
// ------------------------------------------------------------------------------
env.chooseEnvVarPrompt = function (envVars, envVar, cb) {
  var envVarIndex = -1
  ...
  if (typeof envVar === 'string' && envVar !== '') {
    envVarIndex = envVarInArray(envVars, envVar)
    if (envVarIndex >= 0) return cb(null, envVars[envVarIndex])

    // No match was found. Error out.
    return cb({ code: 'NO_MATCHING_ENV_VAR' })
  }

  if (envVars.length === 1) {
    var selectedEnvVar = envVars[0]
    xervo.io.prompt.get([{
      name: 'confirm',
      description: 'Are you sure you want to use environment variable ' + selectedEnvVar.name.data + '?',
      ...
      if (result.confirm === 'no') {
        xervo.io.print('No environment variable chosen.')
        return cb(null, null)
      }

      return cb(null, selectedEnvVar)
    })
  } else {
    xervo.io.print('Please choose which environment variable to use:'.input)
    for (var i = 0, len = envVars.length; i < len; i++) {
      var index = i + 1
      xervo.io.print('  ' + index.toString().input + ') '.input + envVars[i].name.input)
    }
    xervo.io.prompt.get([{
      name: 'environmentVar',
      description: 'Environment variable Number?',
      warning: 'Environment variable number has to be between 1 and ' + envVars.length,
      required: true,
      type: 'number',
      minimum: 1,
      maximum: envVars.length
    }], function (err, result) {
      if (err) {
        return error.handlePromptError(err, cb)
      }
      cb(null, envVars[result.environmentVar - 1])
    })
  }
}
